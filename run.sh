...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        -h|--help) show_help ;;
        -p) USE_PLUS_VERSION=true; shift ;;
        -d) DOMAIN="$2"; shift 2 ;;
        -a) TARGET="$2"; shift 2 ;;
        -ha) LISTEN_HOST="$2"; shift 2 ;;
        -hp) LISTEN_PORT="$2"; shift 2 ;;
        -mc) MAX_CONNECTIONS="$2"; shift 2 ;;
        -t) IDLE_TIMEOUT="$2"; shift 2 ;;
        -c) CLIENT_COUNT="$2"; shift 2 ;;
        -rc) RESOLVERS_PER_CLIENT="$2"; shift 2 ;;
        -hc) HC_URL="$2"; shift 2 ;;
        -hi) HC_INTERVAL="$2"; shift 2 ;;
        -ht) HC_THRESHOLD="$2"; shift 2 ;;
        -ft) RESOLVER_FILE_THRESHOLD="$2"; shift 2 ;;
        -f)
            FILE_MODE=true
        ...
        test_client_connection() {
            ...
            for ((i=0; i<10; i++)); do
                ...
                if grep -q "Connection ready" "test_client.log"; then
                    kill $TEST_PID 2>/dev/null; return 0
                fi
                ! kill -0 $TEST_PID 2>/dev/null && break
            done
            kill $TEST_PID 2>/dev/null; return 1
        }

        start_server
        client_fails=0; connection_fails=0; http_cycle=0; http_fails=0
        while true; do
            ...
            if curl -s -m 5 -k "$HC_URL" >/dev/null; then http_fails=0; else
                ...
                if [ "$http_fails" -ge "$HC_THRESHOLD" ]; then
                    kill $CHECK_PID 2>/dev/null; http_fails=0
                fi
            fi
            ((http_cycle++))
            if [ $((http_cycle % 5)) -eq 0 ]; then
                if test_client_connection; then connection_fails=0; else
                    ...
                    if [ "$connection_fails" -ge 5 ]; then
                        kill $CHECK_PID 2>/dev/null; kill $SERVER_PID 2>/dev/null
                        wait $SERVER_PID 2>/dev/null; start_server
                        connection_fails=0; http_cycle=0
                    fi
                fi
            fi
            sleep 10
